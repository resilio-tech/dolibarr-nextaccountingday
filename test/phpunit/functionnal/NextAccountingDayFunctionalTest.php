...
/**
 * \file    test/phpunit/NextAccountingDayFunctionalTest.php
...
/**
 * Class NextAccountingDayFunctionalTest
 *
 ...
 *
 * @package Testnextaccountingday
 */
class NextAccountingDayFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	private static $module_id = 530000; // TODO: autodetect?
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modNextAccountingDay')->click();
		} else {
			// Disable the module
			$this->byHref('modNextAccountingDay')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/nextaccountingday/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('nextaccountingday/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/nextaccountingday/admin/about.php');
		$this->authenticate();
		return $this->assertContains('nextaccountingday/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('nextaccountingdaywidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modNextAccountingDay_NextAccountingDayTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modNextAccountingDay_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
